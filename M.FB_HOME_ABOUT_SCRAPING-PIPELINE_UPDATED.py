...
#         self.driver = webdriver.Remote(command_executor="http://localhost:4444/wd/hub", desired_capabilities=options.to_capabilities())
        ...
        self.driver.find_element_by_xpath('//*[@id="m_login_email"]').send_keys('robin.jos@mail.ru')
        # self.driver.find_element_by_xpath('//*[@id="m_login_password"]').send_keys('johnsonMLAI2304@007')
        self.driver.find_element_by_xpath('//*[@name="pass"]').send_keys('ygfde5363577%Y')
        self.driver.find_element_by_xpath('//*[@name="pass"]').send_keys(Keys.ENTER)
        ...
        # self.driver.find_element_by_xpath('//*[@id="root"]/table/tbody/tr/td/div/div[3]/a').click()
        sleep(5)
    ...
    def result(self, query):
        ...
        if 'Events' in without_empty_strings:
            ...
            list = ['Friends', 'Photos', 'Life events', 'Did You Know', 'Videos', 'Check_ins', 'Sports', 'Music',
                    'Films', 'Keyboard shortcut help']
            index1 = []
            for i in list:
                #     print(i)
                for j in info1:
            ...
            if len(index1) > 0:
                ...
                if ">Work<" in self.driver.page_source:
                    text_index = self.driver.page_source.index(">Work<") 
                    text = self.driver.page_source[text_index:]
                    if ">Education<" in text:
                        text_index1 = text.index(">Education<")
                        work = text[:text_index1]
                    elif ">Places She\'s Lived<" in text:
                        text_index1 = text.index(">Places She\'s Lived<")
                        work = text[:text_index1]
                    elif ">Places he\'s lived<" in text:
                        text_index1 = text.index(">Places he\'s lived<")
                        work = text[:text_index1]
                    elif ">Contact Info<" in text:
                        text_index1 = text.index(">Contact Info<")
                        work = text[:text_index1]
                    elif ">Basic info<" in text:
                        text_index1 = text.index(">Basic info<")
                        work = text[:text_index1]
                    work_list = work.split("\'https\\3a //scontent.fhyd2-1.fna.fbcdn.net/v")
                    work_list = work_list[1:]
                    Profile_Work = []
                    for work in work_list:
                        test_list1 = re.findall(r'>(.*?)<',work)
                        without_empty_strings1 = [string for string in test_list1 if string != ""]
#                         print(without_empty_strings1)
                        Profile_Work.append(without_empty_strings1)
                    Personal_Info['Profile_Work'] = Profile_Work
                else:
                    Personal_Info['Profile_Work'] = "No_Profile_Work_Details"
                if ">Education<" in self.driver.page_source:
                    text_index = self.driver.page_source.index(">Education<") 
                    text = self.driver.page_source[text_index:]
                    if ">Places She\'s Lived<" in text:
                        text_index1 = text.index(">Places She\'s Lived<")
                        Education= text[:text_index1]
                    elif ">Places he\'s lived<" in text:
                        text_index1 = text.index(">Places he\'s lived<")
                        Education = text[:text_index1]
                    elif ">Contact Info<" in text:
                        ...
                        text_index1 = text.index(">Basic info<")
                        Education = text[:text_index1]
                    Education_list = Education.split("\'https\\3a //scontent.fhyd2-1.fna.fbcdn.net/v")
                    Education_list = Education_list[1:]
                    Profile_Education = []
                    for Education in Education_list:
                        test_list1 = re.findall(r'>(.*?)<',Education)
                        without_empty_strings1 = [string for string in test_list1 if string != ""]
#                         print(without_empty_strings1)
                        Profile_Education.append(without_empty_strings1)
                    Personal_Info['Profile_Education'] = Profile_Education
                else:
                    Personal_Info['Profile_Education'] = "No_Profile_Education_Details"
                        ...
                        for a in Relation_ships:
                            ...
                            if "['/profile.php" in user_name1:
                                ...
                                if "scrapbooks" not in user_name1:
                                    ...
                                    Full_Url = "No_Url"
                                    Relation_Ships_data.append(Full_Url)
                        Relation_Ships_data1 = [Relation_Ships_data[3 * i:3 * i + 3] for i in
                                                range(0, math.ceil(len(Relation_Ships_data) / 3))]
                    else:
                ...
                Family_Member_Relation_stopwords = ['Cousin', 'Aunt', 'Friend', 'Father', 'Mother', 'Brother', 'Sister',
                                                    'Son', 'Niece', 'Daughter', 'Auntie', 'Uncle', 'Nephew',
                                                    'Cousin(female)', 'Cousin(male)', 'Grandfather', 'Grandmother',
                                                    'Granddaughter', 'Grandson', 'Srepsister', 'Stepbrother',
                ...
                if "Family Members" in Info:
                    ...
                    Family_Member_Relati = [Family_Member_Relation.append(e) for e in Info5 if
                                            e in Family_Member_Relation_stopwords]
                        ...
                        for a in Family_Members:
                            ...
                            if "['/profile.php" in user_name1:
                                ...
                                if "scrapbooks" not in user_name1:
                                    ...
                                    Full_Url = "No_Url"
                                    Family_Members_data.append(Full_Url)
                        Family_Members_data1 = [Family_Members_data[3 * i:3 * i + 3] for i in
                                                range(0, math.ceil(len(Family_Members_data) / 3))]
                    else:
...
if __name__ == '__main__':
    ...
    # result = obj.result(query='cfghfghjgfyjgthmrdncfnh')
